...
class Domain_details_visitor extends Home
{
    ...
    */
    ...
    public function display_in_dashboard($id=0,$dashboard='1')
    {
        $count_row=$this->basic->count_row("domain",array("where"=>array("user_id"=>$this->user_id,"dashboard"=>'1')));
        $count=isset($count_row[0]['total_rows']) ? $count_row[0]['total_rows'] : 0;

        if($count==4 && $dashboard=='1')
        {
            $this->session->set_flashdata('dashboard_msg',"<div class='alert alert-warning text-center'><i class='fa fa-remove'></i> ".$this->lang->line("you can not add more domain to dashboard")."</div>");
            redirect('domain_details_visitor/domain_list_visitor');
        }

        $this->basic->update_data("domain",array("user_id"=>$this->user_id,"id"=>$id),array("dashboard"=>$dashboard));

        if($dashboard=='1')
        $this->session->set_flashdata('dashboard_msg',"<div class='alert alert-success text-center'><i class='fa fa-check'></i> ".$this->lang->line("domain has been added to display in dashboard")."</div>");
        else $this->session->set_flashdata('dashboard_msg',"<div class='alert alert-success text-center'><i class='fa fa-check'></i> ".$this->lang->line("domain has been removed dashboard")."</div>");
