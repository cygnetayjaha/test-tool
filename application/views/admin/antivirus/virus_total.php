...
<section class="content-header">
	<h1 class = 'text-info'> <?php echo $this->lang->line("VirusTotal Scan");?></h1>
</section>
	...
	<div class="row" >
		<div class="col-xs-12">
			<div class="grid_container" style="width:100%; min-height:760px;">
				...
				url="<?php echo base_url()."antivirus/virus_total_scan_data"; ?>" 
				...
				>
					...
					<thead>
						<tr>
							...
							<th field="positives" sortable="true">Positives</th>
							<th field="total" sortable="true">Total Scan</th>
							<th field="scanned_at" sortable="true">Scanned at</th>
							<th field="report"><?php echo $this->lang->line("actions") ?></th>
						</tr>
					</thead>
				</table>                        
			</div>
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<div class="modal-header">
				...
				<h4 id="new_search_details_title" class="modal-title"><i class="fa fa-shield"></i> <?php echo $this->lang->line("VirusTotal Scan");?></h4>
			</div><br/>


			<div id="new_search_view_body" class="modal-body">
				 <!-- <div class="row"> -->
				 	<form class="form-horizontal" method="POST" nctype="multipart/form-data"  id="new_search_form" style="margin-bottom:10px">
						<div class="form-group">
							<label class="col-sm-3 col-md-3 control-label">Domain Name</label>
							<div class="col-sm-9 col-md-5">
								<input name="domain_name" id="bulk_file" value="" class="form-control" type="text">									
							</div>
							<div class="col-sm-12 col-md-4">
								<button type="button"  id="new_search_button" class="btn btn-info"><i class="fa fa-search"></i> <?php echo $this->lang->line("start scanning"); ?></button>
							</div>
						</div>        
					</form>
				 <!-- </div> -->
...
<script>
	...
	// $(".virus_total_report").on('click',function(){
	$j(document.body).on('click','.virus_total_report',function(){
				
		var id=$(this).attr('data-id');	
		
		$("#live_data2").html('<img class="center-block" src="'+base_url+'assets/pre-loader/Fancy pants.gif" alt="Processing..."><br/>');
	...
	$(".delete").click(function(){
		...
		if(result)
		{
			...
			var base_url="<?php echo base_url(); ?>";		
			var url = "<?php echo site_url('antivirus/virus_total_scan_delete');?>";
	        var rows = $j("#tt").datagrid("getSelections");
	        var info=JSON.stringify(rows); 
	...
	function get_bulk_progress()
	{
		var base_url="<?php echo base_url(); ?>";			
		$.ajax({
			url:base_url+'antivirus/virus_total_bulk_scan_progress_count',
			type:'POST',
			dataType:'json',
			success:function(response){
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<div class="modal-header">
				...
				<h4 id="" class="modal-title"><i class="fa fa-th-large"></i> </i> <?php echo $this->lang->line("VirusTotal Scan");?> - <?php echo $this->lang->line('Report'); ?></h4>
			</div>

			<div class="modal-body" id="live_data2">
			</div>
